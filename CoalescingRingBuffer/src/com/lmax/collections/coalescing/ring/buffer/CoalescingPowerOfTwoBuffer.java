...
public final class CoalescingPowerOfTwoBuffer<K, V> implements CoalescingBuffer<K, V> {
    ...
    @SuppressWarnings("unchecked")
    public CoalescingPowerOfTwoBuffer(int capacity) {
    ...
    @Override public boolean offer(K key, V value) {
        ...
        for (long readPosition = nextRead; readPosition < nextWrite; readPosition++) {
            int index = computeIndex(readPosition);
